...
def i3clients():
    ...
    for ws_num in range(1,11):
        ...
        for window in windows:
            workspace_num = workspace['name'][0:1]
            win_str = '[%s] %s' % (workspace_num, window['name'])
...
def win_menu(clients, l=10):
    ...
    """
    dmenu = subprocess.Popen(['/usr/bin/dmenu', '-y', '500' ,'-x', '384', '-h', '32', '-w', '1152', '-i', '-p', '$', '-f', '-sf', '#ededed', '-nf', '#ededed', '-nb', '#1e1e1e', '-sb', '#1e1e1e', '-fn', 'Source Code Pro-9:style=Bold', '-sb', '#0088CC'],
